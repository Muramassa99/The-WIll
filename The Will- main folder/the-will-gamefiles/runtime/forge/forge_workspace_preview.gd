...
func sync_from_wip(wip: CraftedItemWIP) -> void:
	...
	multimesh.mesh = _build_cell_mesh()
	...
	for index in range(cells.size()):
		...
		multimesh.set_instance_transform(index, cell_transform)
		multimesh.set_instance_color(index, _resolve_material_color(cell.material_variant_id))
	occupied_cells_instance.multimesh = multimesh
...
func _apply_view_tuning() -> void:
	...
	if light != null:
		...
		light.rotation_degrees = tuning.workspace_light_rotation_degrees
	if grid_bounds_instance != null:
