...
const AMOUNT_RATIO_MIN := 0.05
const AMOUNT_RATIO_MAX := 1.0
const AMOUNT_RATIO_STEP := 0.05
...
func set_amount_ratio(next_amount_ratio: float) -> void:
	active_amount_ratio = _normalize_amount_ratio(next_amount_ratio)
	mark_updated()

func adjust_amount_ratio_steps(step_count: int) -> void:
	set_amount_ratio(active_amount_ratio + (float(step_count) * AMOUNT_RATIO_STEP))

func append_empty_volume_stroke() -> Resource:
	var stroke: Resource = _append_volume_stroke_record(PackedVector3Array(), 0.001, active_amount_ratio)
	mark_updated()
	return stroke

func append_sample_volume_stroke() -> Resource:
	var stroke: Resource = _append_volume_stroke_record(
		_build_sample_stroke_points(get_volume_stroke_count()),
...
func append_active_primitive_deposit() -> Array[Resource]:
	var created_strokes: Array[Resource] = []
	...
	for stroke_spec: Dictionary in stroke_specs:
		created_strokes.append(_append_volume_stroke_record(
			...
			float(stroke_spec.get("amount_ratio", active_amount_ratio))
		))
	mark_updated()
...
func append_point_volume_stroke(
	...
	var stroke: Resource = _append_volume_stroke_record(
		...
		active_amount_ratio if amount_ratio <= 0.0 else _normalize_amount_ratio(amount_ratio)
	)
	mark_updated()
	return stroke
...
func append_point_to_volume_stroke(
	...
	var stroke: Resource = _find_volume_stroke_by_id(stroke_id)
	if stroke == null or _is_volume_stroke_committed(stroke_id):
		return false
	...
	if not changed:
		return false
	stroke.set("path_points", path_points)
	if stroke.has_method("normalize"):
		stroke.call("normalize")
	_sync_material_body_from_stroke(stroke)
	mark_updated()
	return true

func clear_volume_strokes() -> void:
	_remove_pending_volume_strokes()
...
func commit_pending_material_bodies_as_layer() -> Resource:
	...
	layer.call("configure_from_material_bodies", forge_layers.size() + 1, pending_bodies)
...
func get_active_tool_label() -> String:
	return "Spline Line" if active_tool_id == TOOL_SPLINE_LINE else "Volume Stroke"
...
func get_amount_ratio_label() -> String:
	return "Amount %.0f%%" % (active_amount_ratio * 100.0)
...
func get_material_usage_summary() -> Dictionary:
	...
	for body: Resource in material_bodies:
		...
		var entry: Dictionary = material_entries.get(material_variant_id, {
		...
		entry["rough_volume_cell_equivalents"] = float(entry.get("rough_volume_cell_equivalents", 0.0)) + float(body.call("get_signed_rough_volume_cell_equivalents"))
		entry["rough_material_centi_units"] = int(entry.get("rough_material_centi_units", 0)) + int(body.call("get_signed_rough_material_centi_units"))
	...
	for material_variant_id: StringName in material_entries.keys():
		...
		var clamped_centi_units: int = maxi(int(entry.get("rough_material_centi_units", 0)), 0)
		var clamped_volume_cell_equivalents: float = maxf(float(entry.get("rough_volume_cell_equivalents", 0.0)), 0.0)
		entry["rough_material_centi_units"] = clamped_centi_units
		entry["rough_material_units"] = float(clamped_centi_units) / float(ForgeV2MaterialBodyScript.MATERIAL_UNIT_SCALE)
		entry["rough_volume_cell_equivalents"] = clamped_volume_cell_equivalents
		material_entries[material_variant_id] = entry
		total_centi_units += clamped_centi_units
		total_volume_cell_equivalents += clamped_volume_cell_equivalents
	for material_variant_id: StringName in material_entries.keys():
		var entry: Dictionary = material_entries[material_variant_id]
	...
	return {
		...
		"total_rough_material_units": float(total_centi_units) / float(ForgeV2MaterialBodyScript.MATERIAL_UNIT_SCALE),
		"materials": material_entries,
	}

func get_material_usage_label() -> String:
	var usage: Dictionary = get_material_usage_summary()
...
func get_status_summary() -> Dictionary:
	return {
		...
		"material_usage_label": get_material_usage_label(),
...
func _append_volume_stroke_record(
	...
	stroke.set("operation_mode", active_operation_mode)
	stroke.set("material_variant_id", active_material_variant_id)
	stroke.set("placement_policy", placement_policy)
	stroke.set("radius_meters", maxf(radius_meters, 0.001))
	stroke.set("amount_ratio", _normalize_amount_ratio(amount_ratio))
	stroke.set("path_points", path_points)
	stroke.set("created_timestamp", Time.get_unix_time_from_system())
	if stroke.has_method("normalize"):
		stroke.call("normalize")
	volume_strokes.append(stroke)
	var body: Resource = _build_material_body_from_stroke(stroke)
...
func _build_material_body_from_stroke(stroke: Resource) -> Resource:
	...
	if stroke != null:
		...
		body.set("amount_ratio", float(stroke.get("amount_ratio")))
...
func _normalize_amount_ratio(amount_ratio: float) -> float:
	var clamped_amount: float = clampf(amount_ratio, AMOUNT_RATIO_MIN, AMOUNT_RATIO_MAX)
	var snapped_steps: int = roundi((clamped_amount - AMOUNT_RATIO_MIN) / AMOUNT_RATIO_STEP)
	return clampf(
		AMOUNT_RATIO_MIN + (float(snapped_steps) * AMOUNT_RATIO_STEP),
		AMOUNT_RATIO_MIN,
		AMOUNT_RATIO_MAX
	)
...
func _normalize_volume_strokes() -> void:
	var normalized_strokes: Array[Resource] = []
	for stroke: Resource in volume_strokes:
		if stroke == null:
			continue
		if stroke.has_method("normalize"):
	...
	volume_strokes = normalized_strokes
