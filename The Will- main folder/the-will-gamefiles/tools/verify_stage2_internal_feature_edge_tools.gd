...
const RESULT_FILE_PATH := "C:/WORKSPACE/stage2_internal_feature_edge_results.txt"
...
func _run_verification() -> void:
	...
	crafting_ui.open_for(player, forge_controller, "Stage2 Internal Feature Edge Bench")
	await process_frame
	await process_frame
	...
	var has_feature_edge_fillet_tool: bool = _popup_has_item_text(geometry_popup, "Feature Edge Fillet Tool")
	var has_feature_edge_chamfer_tool: bool = _popup_has_item_text(geometry_popup, "Feature Edge Chamfer Tool")

	crafting_ui.call("_on_action_menu_id_pressed", int(menu_ids.get("geometry_tool_surface_feature_edge_fillet", -1)))
	await process_frame
	await process_frame

	var grip_safe_target: Dictionary = _resolve_visible_internal_feature_edge_target(
		preview,
	...
	var general_target: Dictionary = _resolve_visible_internal_feature_edge_target(
		preview,
		forge_controller.active_wip,
		stage2_item_state,
	...
	var feature_edge_tool_apply_visible: bool = _popup_has_item_text(geometry_popup, "Apply Selected Targets")
	var feature_edge_tool_clear_visible: bool = _popup_has_item_text(geometry_popup, "Clear Target Selection")
	...
	var grip_safe_selected_count: int = PackedStringArray(crafting_ui.get("stage2_selected_patch_ids")).size()
	var origins_before_safe_feature_edge_fillet: Array[Vector3] = _collect_patch_origins(stage2_item_state)
	crafting_ui.call("_on_action_menu_id_pressed", int(menu_ids.get("geometry_selection_apply", -1)))
	await process_frame
	var grip_safe_feature_edge_fillet_changed_shell: bool = _has_any_patch_origin_changed(
		origins_before_safe_feature_edge_fillet,
	...
	crafting_ui.call("_on_action_menu_id_pressed", int(menu_ids.get("geometry_tool_surface_feature_edge_chamfer", -1)))
	await process_frame
	await process_frame
	crafting_ui.call("_toggle_stage2_patch_selection_at_screen_position", grip_safe_screen_position)
	await process_frame
	var origins_before_safe_feature_edge_chamfer: Array[Vector3] = _collect_patch_origins(stage2_item_state)
	crafting_ui.call("_on_action_menu_id_pressed", int(menu_ids.get("geometry_selection_apply", -1)))
	await process_frame
	var grip_safe_feature_edge_chamfer_changed_shell: bool = _has_any_patch_origin_changed(
		origins_before_safe_feature_edge_chamfer,
		_collect_patch_origins(stage2_item_state)
	)
	...
	var origins_before_general_feature_edge_chamfer: Array[Vector3] = _collect_patch_origins(stage2_item_state)
	crafting_ui.call("_on_action_menu_id_pressed", int(menu_ids.get("geometry_selection_apply", -1)))
	await process_frame
	var general_feature_edge_chamfer_changed_shell: bool = _has_any_patch_origin_changed(
		origins_before_general_feature_edge_chamfer,
		_collect_patch_origins(stage2_item_state)
	)

	var lines: PackedStringArray = []
	lines.append("stage2_geometry_menu_has_feature_edge_fillet=%s" % str(has_feature_edge_fillet_tool))
	lines.append("stage2_geometry_menu_has_feature_edge_chamfer=%s" % str(has_feature_edge_chamfer_tool))
	lines.append("feature_edge_tool_apply_visible=%s" % str(feature_edge_tool_apply_visible))
	lines.append("feature_edge_tool_clear_visible=%s" % str(feature_edge_tool_clear_visible))
	...
	lines.append("grip_safe_feature_edge_fillet_changed_shell=%s" % str(grip_safe_feature_edge_fillet_changed_shell))
	lines.append("selected_count_after_clear=%d" % selected_count_after_clear)
	lines.append("grip_safe_feature_edge_chamfer_changed_shell=%s" % str(grip_safe_feature_edge_chamfer_changed_shell))
	lines.append("general_selected_count=%d" % general_selected_count)
	lines.append("general_feature_edge_chamfer_changed_shell=%s" % str(general_feature_edge_chamfer_changed_shell))
...
func _build_front_heavy_sword_wip() -> CraftedItemWIP:
	...
	wip.wip_id = &"verify_stage2_internal_feature_edge_front_heavy_sword"
	wip.forge_project_name = "Verify Stage2 Internal Feature Edge Front Heavy Sword"
...
func _resolve_visible_internal_feature_edge_target(
	...
	for patch_state in stage2_item_state.patch_states:
		...
		if patch_ids.size() <= 1:
			continue
		if not _patch_ids_all_in_zone(stage2_item_state, patch_ids, zone_mask_id):
			continue
...
func _patch_ids_all_in_zone(stage2_item_state, patch_ids: PackedStringArray, zone_mask_id: StringName) -> bool:
	...
	for patch_id_string: String in patch_ids:
		var patch_state = patch_lookup.get(StringName(patch_id_string), null)
		if patch_state == null or patch_state.zone_mask_id != zone_mask_id:
			return false
	return true
