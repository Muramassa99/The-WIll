...
const ShapeClassifierResolverScript = preload("res://core/resolvers/shape_classifier_resolver.gd")

const OUTPUT_PATH := "c:/WORKSPACE/saved_wip_rule_diagnosis.txt"
const TARGET_PROJECT_NAME := "test_sword"

var shape_classifier_resolver = ShapeClassifierResolverScript.new()
...
func _run_diagnosis() -> void:
	...
	if target_wip != null:
		...
		lines.append("profile_primary_grip_contact_position=%s" % str(profile.primary_grip_contact_position if profile != null else Vector3.ZERO))
		lines.append("profile_primary_grip_span_start=%s" % str(profile.primary_grip_span_start if profile != null else Vector3.ZERO))
		lines.append("profile_primary_grip_span_end=%s" % str(profile.primary_grip_span_end if profile != null else Vector3.ZERO))
		lines.append("profile_primary_grip_span_length_voxels=%d" % int(profile.primary_grip_span_length_voxels if profile != null else 0))
		lines.append("profile_edge_score=%s" % str(profile.edge_score if profile != null else 0.0))
		lines.append("profile_blunt_score=%s" % str(profile.blunt_score if profile != null else 0.0))
		lines.append("profile_pierce_score=%s" % str(profile.pierce_score if profile != null else 0.0))
		lines.append("profile_guard_score=%s" % str(profile.guard_score if profile != null else 0.0))
		lines.append("profile_flex_score=%s" % str(profile.flex_score if profile != null else 0.0))
		lines.append("profile_launch_score=%s" % str(profile.launch_score if profile != null else 0.0))
		...
		for anchor_index: int in range(anchors.size()):
			var anchor: AnchorAtom = anchors[anchor_index]
			if anchor == null:
				continue
			lines.append("--- anchor_%d ---" % anchor_index)
			lines.append("anchor_id=%s" % String(anchor.anchor_id))
			lines.append("anchor_type=%s" % anchor.anchor_type)
			lines.append("anchor_local_position=%s" % str(anchor.local_position))
			lines.append("anchor_local_axis=%s" % str(anchor.local_axis))
			lines.append("anchor_span_length=%d" % anchor.span_length)
			lines.append("anchor_span_start_index=%d" % anchor.span_start_index)
			lines.append("anchor_span_end_index=%d" % anchor.span_end_index)
			lines.append("anchor_span_start_position=%s" % str(anchor.span_start_local_position))
			lines.append("anchor_span_end_position=%s" % str(anchor.span_end_local_position))
			lines.append("anchor_span_material_ratio=%s" % str(anchor.span_anchor_material_ratio))
		...
		for segment_index: int in range(segments.size()):
			...
			lines.append("joint_support_material_ratio=%s" % str(segment.joint_support_material_ratio))
			lines.append("bow_string_material_ratio=%s" % str(segment.bow_string_material_ratio))
			...
			lines.append("material_mix=%s" % str(segment.material_mix))
			lines.append("shape_edge_valid_hint=%s" % str(shape_classifier_resolver.is_edge_valid_segment(segment)))
			lines.append("shape_blunt_valid_hint=%s" % str(shape_classifier_resolver.is_blunt_valid_segment(segment)))
			lines.append("shape_guard_span_hint=%s" % str(shape_classifier_resolver.get_guard_span_score(segment)))
			lines.append("shape_pierce_tip_hint=%s" % str(shape_classifier_resolver.get_pierce_tip_hint_score(segment)))
			lines.append("grip_envelope_ok=%s" % str(_is_grip_envelope_ok(segment, rules)))
			lines.append("grip_anchor_ratio_ok=%s" % str(segment.anchor_material_ratio >= rules.primary_grip_min_anchor_ratio))
			lines.append("grip_endcaps_ok=%s" % str(segment.start_slice_anchor_valid and segment.end_slice_anchor_valid))
			lines.append("grip_profile_ok=%s" % str(_is_grip_profile_ok(segment)))
			lines.append("grip_edge_overlap_ok=%s" % str(not segment.edge_span_overlap))
			lines.append("grip_candidate_passes_all=%s" % str(_passes_all_grip_gates(segment, rules)))
...
func _is_grip_envelope_ok(segment: SegmentAtom, rules: ForgeRulesDef) -> bool:
	return (
...
func _is_grip_profile_ok(segment: SegmentAtom) -> bool:
	return (
...
func _passes_all_grip_gates(segment: SegmentAtom, rules: ForgeRulesDef) -> bool:
	return (
		_is_grip_envelope_ok(segment, rules)
		and segment.anchor_material_ratio >= rules.primary_grip_min_anchor_ratio
		and segment.start_slice_anchor_valid
		and segment.end_slice_anchor_valid
		and _is_grip_profile_ok(segment)
		and not segment.edge_span_overlap
	)
