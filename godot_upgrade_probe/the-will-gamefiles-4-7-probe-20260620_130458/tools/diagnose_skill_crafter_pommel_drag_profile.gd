...
const PREVIEW_REFRESH_INTERVAL_SECONDS := 1.0 / 60.0
...
func _run_drag_pass(ui: CombatAnimationStationUI, pass_label: String, debug_enabled: bool, path_kind: String) -> void:
	...
	ui.motion_node_editor.begin_drag(&"pommel", Vector2.ZERO, source_motion_node)
	...
	for sample_index: int in range(sample_count):
		...
		ui.call("_queue_pommel_drag_authority_target", requested_pommel)
		var t1: int = Time.get_ticks_usec()
		var changed: bool = false
		var t2: int = t1
		var t3: int = Time.get_ticks_usec()
		row["resolve_segment_ms"] = _elapsed_ms(t0, t1)
		row["authority_solve_ms"] = 0.0
		...
		var refresh_interval_seconds: float = float(ui.call("_get_preview_drag_refresh_interval_msec")) / 1000.0
		var refresh_due: bool = (
			last_refresh_usec <= 0
			or _elapsed_seconds(last_refresh_usec, Time.get_ticks_usec()) >= maxf(refresh_interval_seconds, PREVIEW_REFRESH_INTERVAL_SECONDS)
		)
		if refresh_due:
			refresh_count += 1
			var refresh_start_usec: int = Time.get_ticks_usec()
			var solve_start_usec: int = Time.get_ticks_usec()
			var authority_result: Dictionary = ui.call("_resolve_pending_pommel_drag_authority", true) as Dictionary
			var solve_end_usec: int = Time.get_ticks_usec()
			row["authority_solve_ms"] = _elapsed_ms(solve_start_usec, solve_end_usec)
			changed = bool(authority_result.get("changed", false))
			row["changed"] = changed
			row["authority_legal"] = bool(authority_result.get("legal", true))
			row["authority_reason"] = String(authority_result.get("reason", ""))
			var refresh_profile: Dictionary = _profile_refresh_preview_scene(ui)
			last_refresh_usec = Time.get_ticks_usec()
			ui.preview_drag_last_refresh_cost_msec = int(round(_elapsed_ms(refresh_start_usec, last_refresh_usec)))
	...
	var release_authority_result: Dictionary = ui.call("_resolve_pending_pommel_drag_authority", true) as Dictionary
	ui.motion_node_editor.end_drag()
	...
	lines.append("pass_%s_release_authority_legal=%s" % [pass_label, str(bool(release_authority_result.get("legal", true)))])
	lines.append("pass_%s_release_reused_drag_authority=%s" % [pass_label, str(bool(release_result.get("reused_drag_authority", false)))])
...
func _profile_refresh_preview_scene(ui: CombatAnimationStationUI) -> Dictionary:
	...
	if ui.preview_drag_override_node != null:
		...
		var drag_chain_cache: Dictionary = ui.call("_ensure_preview_drag_motion_chain_cache") as Dictionary
		...
		if bool(ui.call("_can_reuse_pommel_drag_authority_commit")):
			playback_state["authoring_drag_endpoint_authority_prevalidated"] = true
			playback_state["authoring_drag_endpoint_validation_result"] = ui.preview_drag_pommel_last_validation_result
	...
	var effective_chain: Array = cached_effective_chain
	if effective_chain.is_empty():
		effective_chain = presenter.call(
	...
	var visible_chain: Array = cached_visible_chain
	if visible_chain.is_empty():
		visible_chain = presenter.call("_build_visible_motion_node_chain", active_draft, effective_chain) as Array
	var visible_selected_index: int = (
		int(playback_state.get("authoring_drag_visible_selected_node_index", -1))
		if not cached_visible_chain.is_empty()
		else int(presenter.call(
	...
	if use_open_mount_baseline:
		...
		resolved_playback_state = presenter.call(
			...
			true,
			active_draft,
			ui.preview_drag_override_node != null
		) as Dictionary
	...
	var display_chain: Array = visible_chain
	if not (bool(resolved_playback_state.get("authoring_drag_budgeted_visuals", false)) and ui.preview_drag_override_node != null):
		display_chain = presenter.call(
...
func _append_pass_summary(pass_label: String, samples: Array[Dictionary]) -> void:
	...
	_append_stage_summary(pass_label, refresh_rows, "authority_solve_ms")
	_append_stage_summary(pass_label, refresh_rows, "apply_pose_ms")
	_append_stage_summary(pass_label, refresh_rows, "display_chain_ms")
...
func _append_pass_samples(pass_label: String, samples: Array[Dictionary]) -> void:
	lines.append("pass_%s_samples_csv=sample,wall_ms,sample_total_ms,resolve_segment_ms,authority_solve_ms,apply_segment_ms,normalize_ms,authored_pommel_error_meters,actual_pommel_error_meters,display_pommel_error_meters,marker_pommel_error_meters,resolved_pommel_error_meters,refresh_ran,refresh_total_ms,apply_pose_ms,visuals_ms,debug_collision_ms,refresh_actor_weapon_ms,open_mount_seed_ms,build_chains_ms,authority_legal,endpoint_legal,endpoint_dominant_correction_meters,endpoint_support_correction_meters,endpoint_path_illegal,endpoint_point_illegal,endpoint_front_bias_failed,endpoint_alternate_correction_disabled,endpoint_weapon_body_illegal,endpoint_weapon_body_region,collision_pose_legal,collision_pose_region,collision_pose_sample,contact_clearance_reason,final_anchor_reason,pre_anchor_grip_error_meters,post_anchor_separation_delta_meters,weapon_tip_alignment_error_meters,weapon_pommel_alignment_error_meters" % pass_label)
	for row in samples:
		lines.append("pass_%s_sample=%d,%.3f,%.3f,%.3f,%.3f,%.3f,%.3f,%.5f,%.5f,%.5f,%.5f,%.5f,%s,%.3f,%.3f,%.3f,%.3f,%.3f,%.3f,%.3f,%s,%s,%.5f,%.5f,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%.5f,%.5f,%.5f,%.5f" % [
			pass_label,
			int(row.get("sample", -1)),
			float(row.get("wall_ms", 0.0)),
			float(row.get("sample_total_ms", 0.0)),
			float(row.get("resolve_segment_ms", 0.0)),
			float(row.get("authority_solve_ms", 0.0)),
			...
			str(bool(row.get("authority_legal", true))),
