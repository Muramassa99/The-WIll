...
func _run_verification() -> void:
	...
	var degenerate_chain: Array = [
		_build_reset_like_motion_node(0, 0.01),
		_build_reset_like_motion_node(1, 0.25),
		_build_reset_like_motion_node(2, 0.25),
	]
	var degenerate_clip = baker.bake_from_motion_node_chain(
		degenerate_chain,
		{
			"clip_kind": CombatRuntimeClipScript.CLIP_KIND_SKILL_PLAYBACK,
			"source_draft_id": &"verify_reset_like_runtime_clip",
			"source_skill_slot_id": &"skill_slot_reset",
			"source_equipment_slot_id": &"hand_right",
			"source_weapon_wip_id": &"verify_weapon",
			"source_weapon_length_meters": 0.7,
			"sample_rate_hz": 30.0,
	...
	var degenerate_clip_player: CombatAnimationChainPlayer = CombatAnimationChainPlayerScript.new()
	degenerate_clip_player.prepare_runtime_clip(degenerate_clip, 1.0, false)
	degenerate_clip_player.start()
	degenerate_clip_player.advance(0.125)
	var degenerate_runtime_sample_length: float = degenerate_clip_player.current_tip_position.distance_to(degenerate_clip_player.current_pommel_position)
	...
	var grip_slide_samples_match: bool = (
		clip != null
		and clip.get_frame_count() > 0
		and clip.baked_grip_seat_slide_offsets.size() == clip.get_frame_count()
		and clip.baked_secondary_grip_seat_slide_offsets.size() == clip.get_frame_count()
		and absf(clip.baked_grip_seat_slide_offsets[clip.get_frame_count() - 1] - final_node.grip_seat_slide_offset) <= 0.0001
		and absf(clip.baked_secondary_grip_seat_slide_offsets[clip.get_frame_count() - 1] - final_node.secondary_grip_seat_slide_offset) <= 0.0001
	)
	var clip_origin_ids_ok: bool = (
		clip != null
	...
	var clip_player_origin_ids_ok: bool = (
		clip_player.current_tip_position_origin_id == CombatOriginRecordScript.ORIGIN_TRAJECTORY_AUTHORING
		and clip_player.current_pommel_position_origin_id == CombatOriginRecordScript.ORIGIN_TRAJECTORY_AUTHORING
		and clip_player.current_contact_grip_axis_origin_id == CombatOriginRecordScript.ORIGIN_TRAJECTORY_AUTHORING
		and clip_player.current_solved_replay_reference_origin_id == CombatOriginRecordScript.ORIGIN_SOLVED_REPLAY_REFERENCE
		and clip_player.current_solved_weapon_reference_origin_id == CombatOriginRecordScript.ORIGIN_SOLVED_REPLAY_REFERENCE
		and clip_player.current_solved_anchor_origin_id == CombatOriginRecordScript.ORIGIN_WEAPON_ROOT
	)
	var degenerate_final_tip: Vector3 = (
		degenerate_clip.baked_tip_positions_local[degenerate_clip.baked_tip_positions_local.size() - 1]
		if degenerate_clip != null and not degenerate_clip.baked_tip_positions_local.is_empty()
		else Vector3.INF
	)
	var degenerate_final_node: CombatAnimationMotionNode = degenerate_chain[degenerate_chain.size() - 1] as CombatAnimationMotionNode
	var degenerate_curve_bake_ok: bool = (
		degenerate_clip != null
		and degenerate_clip.is_playable()
		and degenerate_clip.get_frame_count() > 2
		and degenerate_final_tip.distance_to(degenerate_final_node.tip_position_local) <= 0.0001
		and absf(degenerate_runtime_sample_length - 0.7) <= 0.0001
		and degenerate_clip_player.current_tip_position_origin_id == CombatOriginRecordScript.ORIGIN_TRAJECTORY_AUTHORING
	)
	var all_checks_passed: bool = (
		clip != null
		and clip.is_playable()
		and clip_origin_ids_ok
		and clip_player_origin_ids_ok
		and degenerate_curve_bake_ok
	...
	lines.append("clip_origin_ids_ok=%s" % str(clip_origin_ids_ok))
	lines.append("clip_player_origin_ids_ok=%s" % str(clip_player_origin_ids_ok))
	...
	lines.append("clip_grip_slide_samples_match=%s" % str(grip_slide_samples_match))
	lines.append("clip_player_primary_grip_slide=%.2f" % clip_player.current_grip_seat_slide)
	lines.append("clip_player_secondary_grip_slide=%.2f" % clip_player.current_secondary_grip_seat_slide)
	...
	lines.append("degenerate_clip_exists=%s" % str(degenerate_clip != null))
	lines.append("degenerate_clip_playable=%s" % str(degenerate_clip != null and degenerate_clip.is_playable()))
	lines.append("degenerate_clip_frame_count=%d" % int(degenerate_clip.get_frame_count() if degenerate_clip != null else 0))
	lines.append("degenerate_clip_final_tip_matches=%s" % str(degenerate_final_tip.distance_to(degenerate_final_node.tip_position_local) <= 0.0001))
	lines.append("degenerate_clip_player_runtime_length_locked=%s" % str(absf(degenerate_runtime_sample_length - 0.7) <= 0.0001))
	lines.append("degenerate_curve_bake_ok=%s" % str(degenerate_curve_bake_ok))
...
func _build_motion_node(
	...
	motion_node.grip_seat_slide_offset = 0.2 + float(node_index) * 0.1
	motion_node.secondary_grip_seat_slide_offset = -0.1 + float(node_index) * 0.15
...
func _build_reset_like_motion_node(
	node_index: int,
	transition_duration_seconds: float
) -> CombatAnimationMotionNode:
	var motion_node: CombatAnimationMotionNode = _build_motion_node(
		node_index,
