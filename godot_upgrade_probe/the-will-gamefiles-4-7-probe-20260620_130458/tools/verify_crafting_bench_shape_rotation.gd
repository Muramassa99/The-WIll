...
func _run_verification() -> void:
	...
	var geometry_menu_has_rotate_left: bool = _popup_has_item_text(geometry_popup, "Rotate Shape -5°")
	var geometry_menu_has_rotate_right: bool = _popup_has_item_text(geometry_popup, "Rotate Shape +5°")
	var geometry_popup_persists_on_item_selection: bool = not geometry_popup.hide_on_item_selection
	var rectangle_status_zero_ok: bool = crafting_ui.status_text.text.contains("Rectangle Draw (0°)")
	var rectangle_overlay_zero_ok: bool = crafting_ui.rotation_status_label.text == "Rotation: 0°"
	var rotation_step_degrees_ok: bool = (
		shape_presenter.get_rotation_degrees(1) == 5
		and shape_presenter.get_rotation_degrees(18) == 90
		and shape_presenter.get_rotation_degrees(-1) == 355
	)
	...
	crafting_ui.geometry_menu_button.show_popup()
	await process_frame
	crafting_ui.call("_on_action_menu_id_pressed", int(menu_ids.get("geometry_shape_rotate_right", -1)))
	await process_frame
	await process_frame
	crafting_ui.call("_rebuild_geometry_menu")
	var rectangle_rotation_five_ok: bool = _popup_has_item_text(crafting_ui.geometry_menu_button.get_popup(), "Shape Rotation: 5°")
	var rectangle_status_five_ok: bool = crafting_ui.status_text.text.contains("Rectangle Draw (5°)")
	var rectangle_overlay_five_ok: bool = crafting_ui.rotation_status_label.text == "Rotation: 5°"
	var geometry_popup_visible_after_first_rotation: bool = geometry_popup.visible
	crafting_ui.call("_on_action_menu_id_pressed", int(menu_ids.get("geometry_shape_rotate_right", -1)))
	await process_frame
	await process_frame
	crafting_ui.call("_rebuild_geometry_menu")
	var rectangle_rotation_ten_ok: bool = _popup_has_item_text(crafting_ui.geometry_menu_button.get_popup(), "Shape Rotation: 10°")
	var rectangle_overlay_ten_ok: bool = crafting_ui.rotation_status_label.text == "Rotation: 10°"
	var geometry_popup_visible_after_second_rotation: bool = geometry_popup.visible
	crafting_ui.call("_step_structural_shape_rotation", 16)
	await process_frame
	crafting_ui.call("_rebuild_geometry_menu")
	...
	var rectangle_overlay_ninety_ok: bool = crafting_ui.rotation_status_label.text == "Rotation: 90°"
	...
	var rectangle_commit_shape_settings: Dictionary = crafting_ui.call("_get_stage1_shape_settings")
	rectangle_commit_shape_settings["use_fixed_size"] = true
	var rectangle_commit_expected_cells: Array[Vector3i] = shape_presenter.build_shape_footprint(
		...
		forge_controller.grid_size,
		18,
		rectangle_commit_shape_settings
	)
	...
	crafting_ui.call("_step_structural_shape_rotation", -18)
	await process_frame
	...
	lines.append("geometry_popup_persists_on_item_selection=%s" % str(geometry_popup_persists_on_item_selection))
	lines.append("rectangle_status_zero_ok=%s" % str(rectangle_status_zero_ok))
	lines.append("rectangle_overlay_zero_ok=%s" % str(rectangle_overlay_zero_ok))
	lines.append("rotation_step_degrees_ok=%s" % str(rotation_step_degrees_ok))
	lines.append("rectangle_rotation_five_ok=%s" % str(rectangle_rotation_five_ok))
	lines.append("rectangle_status_five_ok=%s" % str(rectangle_status_five_ok))
	lines.append("rectangle_overlay_five_ok=%s" % str(rectangle_overlay_five_ok))
	lines.append("geometry_popup_visible_after_first_rotation=%s" % str(geometry_popup_visible_after_first_rotation))
	lines.append("rectangle_rotation_ten_ok=%s" % str(rectangle_rotation_ten_ok))
	lines.append("rectangle_overlay_ten_ok=%s" % str(rectangle_overlay_ten_ok))
	lines.append("geometry_popup_visible_after_second_rotation=%s" % str(geometry_popup_visible_after_second_rotation))
	...
	lines.append("rectangle_overlay_ninety_ok=%s" % str(rectangle_overlay_ninety_ok))
