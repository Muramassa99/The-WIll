...
func _run_verification() -> void:
	...
	var geometry_menu_has_spline_line: bool = _popup_has_item_text(geometry_popup, "Spline Line")
	var geometry_menu_replaced_pick_material: bool = not _popup_has_item_text(geometry_popup, "Pick Material")
	...
	var spline_result: Dictionary = await _run_spline_line_case(
		crafting_ui,
		forge_controller,
		plane_viewport,
		Vector2i(2, 8),
	...
	lines.append("geometry_menu_has_spline_line=%s" % str(geometry_menu_has_spline_line))
	lines.append("geometry_menu_replaced_pick_material=%s" % str(geometry_menu_replaced_pick_material))
	...
	lines.append("spline_forces_plane_mode=%s" % str(bool(spline_result.get("forces_plane_mode", false))))
	lines.append("spline_confirm_visible=%s" % str(bool(spline_result.get("confirm_visible", false))))
	lines.append("spline_confirm_disabled_until_two_points=%s" % str(bool(spline_result.get("confirm_disabled_until_two_points", false))))
	lines.append("spline_overlay_primary_visible=%s" % str(bool(spline_result.get("overlay_primary_visible", false))))
	lines.append("spline_overlay_secondary_hidden=%s" % str(bool(spline_result.get("overlay_secondary_hidden", false))))
	lines.append("spline_overlay_primary_up_text_ok=%s" % str(bool(spline_result.get("overlay_primary_up_text_ok", false))))
	lines.append("spline_overlay_primary_down_text_ok=%s" % str(bool(spline_result.get("overlay_primary_down_text_ok", false))))
	lines.append("spline_overlay_primary_button_increased=%s" % str(bool(spline_result.get("overlay_primary_button_increased", false))))
	lines.append("spline_anchor_markers_ok=%s" % str(bool(spline_result.get("anchor_markers_ok", false))))
	lines.append("spline_handle_markers_ok=%s" % str(bool(spline_result.get("handle_markers_ok", false))))
	lines.append("spline_width_one_preview_count=%d" % int(spline_result.get("width_one_preview_count", 0)))
	lines.append("spline_width_two_preview_count=%d" % int(spline_result.get("width_two_preview_count", 0)))
	lines.append("spline_width_three_preview_count=%d" % int(spline_result.get("width_three_preview_count", 0)))
	lines.append("spline_width_two_uses_even_split=%s" % str(bool(spline_result.get("width_two_uses_even_split", false))))
	lines.append("spline_width_increase_expanded_preview=%s" % str(bool(spline_result.get("width_increase_expanded_preview", false))))
	lines.append("spline_tool_menu_has_line_end_option=%s" % str(bool(spline_result.get("tool_menu_has_line_end_option", false))))
	lines.append("spline_tool_menu_has_square_end=%s" % str(bool(spline_result.get("tool_menu_has_square_end", false))))
	lines.append("spline_tool_menu_has_rounded_end=%s" % str(bool(spline_result.get("tool_menu_has_rounded_end", false))))
	lines.append("spline_square_end_selected=%s" % str(bool(spline_result.get("square_end_selected", false))))
	lines.append("spline_expected_count=%d" % int(spline_result.get("expected_count", 0)))
	lines.append("spline_placed_count=%d" % int(spline_result.get("placed_count", 0)))
	lines.append("spline_place_expected_cells_ok=%s" % str(bool(spline_result.get("expected_cells_ok", false))))
	lines.append("spline_erase_success=%s" % str(bool(spline_result.get("erase_success", false))))
...
func _run_shape_case(
	...
	var expected_cells: Array[Vector3i] = shape_presenter.build_shape_footprint(
		...
		int(crafting_ui.get("structural_shape_rotation_step")),
		shape_settings
	)
...
func _run_spline_line_case(
	...
	await process_frame
	crafting_ui.armed_material_variant_id = material_id
	crafting_ui.call("_set_active_tool", ForgeWorkspaceShapeToolPresenterScript.FAMILY_SPLINE_LINE)
	await process_frame
	crafting_ui.draw_tool_button.emit_signal("pressed")
	await process_frame
	await process_frame

	var forces_plane_mode: bool = crafting_ui.main_workspace_mode == crafting_ui.WORKSPACE_VIEW_PLANE
	var confirm_visible: bool = crafting_ui.confirm_spline_button.visible
	var confirm_disabled_initially: bool = crafting_ui.confirm_spline_button.disabled
	var spline_overlay_primary_visible: bool = crafting_ui.shape_primary_size_overlay_row.visible
	var spline_overlay_secondary_hidden: bool = not crafting_ui.shape_secondary_size_overlay_row.visible
	var spline_overlay_primary_up_text_ok: bool = crafting_ui.shape_primary_size_up_button.text == "+"
	var spline_overlay_primary_down_text_ok: bool = crafting_ui.shape_primary_size_down_button.text == "-"

	_click_plane_cell(plane_viewport, start_plane_position)
	await process_frame
	await process_frame
	var confirm_disabled_after_one_point: bool = crafting_ui.confirm_spline_button.disabled
	_click_plane_cell(plane_viewport, end_plane_position)
	await process_frame
	await process_frame

	var width_one_preview_cells: Array[Vector3i] = plane_viewport.structural_shape_preview_cells.duplicate()
	var width_one_preview_count: int = width_one_preview_cells.size()
	var confirm_enabled_after_two_points: bool = crafting_ui.confirm_spline_button.visible and not crafting_ui.confirm_spline_button.disabled
	var anchor_markers_ok: bool = plane_viewport.spline_line_anchor_plane_positions.size() == 2
	var handle_markers_ok: bool = (
		plane_viewport.spline_line_in_handle_plane_positions.size() == 2
		and plane_viewport.spline_line_out_handle_plane_positions.size() == 2
		and plane_viewport.spline_line_curve_points.size() >= 2
	)

	crafting_ui.shape_primary_size_up_button.emit_signal("pressed")
	await process_frame
	await process_frame
	var width_two_preview_count: int = plane_viewport.structural_shape_preview_cells.size()
	var spline_overlay_primary_button_increased: bool = crafting_ui.spline_line_width_cells == 2
	crafting_ui.call("_step_stage1_shape_primary_size", 1)
	await process_frame
	await process_frame
	var width_three_preview_cells: Array[Vector3i] = plane_viewport.structural_shape_preview_cells.duplicate()
	var width_three_preview_count: int = width_three_preview_cells.size()

	var tool_popup: PopupMenu = crafting_ui.tool_menu_button.get_popup()
	crafting_ui.call("_rebuild_tool_menu")
	var tool_menu_has_line_end_option: bool = _popup_contains_prefix(tool_popup, "Line End Option:")
	var tool_menu_has_square_end: bool = _popup_has_item_text(tool_popup, "Square End")
	var tool_menu_has_rounded_end: bool = _popup_has_item_text(tool_popup, "Rounded End")

	var menu_ids: Dictionary = crafting_ui.call("_get_action_menu_ids")
	crafting_ui.call("_on_action_menu_id_pressed", int(menu_ids.get("tool_spline_end_cap_square", -1)))
	await process_frame
	await process_frame
	var square_end_selected: bool = crafting_ui.spline_line_end_cap_mode == ForgeWorkspaceShapeToolPresenterScript.SPLINE_END_CAP_SQUARE
	var expected_cells: Array[Vector3i] = plane_viewport.structural_shape_preview_cells.duplicate()

	crafting_ui.confirm_spline_button.emit_signal("pressed")
	await process_frame
	await process_frame
	var placed_count: int = _count_cells(crafting_ui.call("_ensure_wip_for_editing"))
	...
	return {
		"forces_plane_mode": forces_plane_mode,
		"confirm_visible": confirm_visible,
		"confirm_disabled_until_two_points": confirm_disabled_initially and confirm_disabled_after_one_point and confirm_enabled_after_two_points,
		"overlay_primary_visible": spline_overlay_primary_visible,
		"overlay_secondary_hidden": spline_overlay_secondary_hidden,
		"overlay_primary_up_text_ok": spline_overlay_primary_up_text_ok,
		"overlay_primary_down_text_ok": spline_overlay_primary_down_text_ok,
		"overlay_primary_button_increased": spline_overlay_primary_button_increased,
		"anchor_markers_ok": anchor_markers_ok,
		"handle_markers_ok": handle_markers_ok,
		"width_one_preview_count": width_one_preview_count,
		"width_two_preview_count": width_two_preview_count,
		"width_three_preview_count": width_three_preview_count,
		"width_two_uses_even_split": width_two_preview_count == width_one_preview_count * 2,
		"width_increase_expanded_preview": width_three_preview_count > width_one_preview_count,
		"tool_menu_has_line_end_option": tool_menu_has_line_end_option,
		"tool_menu_has_square_end": tool_menu_has_square_end,
		"tool_menu_has_rounded_end": tool_menu_has_rounded_end,
		"square_end_selected": square_end_selected,
		"expected_count": expected_cells.size(),
		"placed_count": placed_count,
		"expected_cells_ok": expected_cells_ok,
		"erase_success": _count_cells(crafting_ui.call("_ensure_wip_for_editing")) == 0,
	}

func _click_plane_cell(plane_viewport: ForgePlaneViewport, plane_position_uv: Vector2i) -> void:
