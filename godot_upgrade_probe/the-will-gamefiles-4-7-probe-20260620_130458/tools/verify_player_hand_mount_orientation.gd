...
func _run_verification() -> void:
	...
	skill_slot_state.save_file_path = "%s/verify_mount_skill_slot_state.tres" % TEMP_STATE_DIR
	...
	var right_normal: Dictionary = await _equip_and_capture(player, right_normal_wip.wip_id, &"hand_right", CraftedItemWIP.GRIP_NORMAL)
	var right_reverse: Dictionary = await _equip_and_capture(player, right_reverse_wip.wip_id, &"hand_right", CraftedItemWIP.GRIP_REVERSE)
	var left_normal: Dictionary = await _equip_and_capture(player, left_normal_wip.wip_id, &"hand_left", CraftedItemWIP.GRIP_NORMAL)
	var left_reverse: Dictionary = await _equip_and_capture(player, left_reverse_wip.wip_id, &"hand_left", CraftedItemWIP.GRIP_REVERSE)
	...
	lines.append("right_normal_tip_axis_score=%.6f" % float(right_normal.get("tip_axis_score", -2.0)))
	lines.append("right_reverse_tip_axis_score=%.6f" % float(right_reverse.get("tip_axis_score", -2.0)))
	lines.append("left_normal_tip_axis_score=%.6f" % float(left_normal.get("tip_axis_score", -2.0)))
	lines.append("left_reverse_tip_axis_score=%.6f" % float(left_reverse.get("tip_axis_score", -2.0)))
	lines.append("right_normal_tip_axis_matches_contact_axis=%s" % str(float(right_normal.get("tip_axis_score", -2.0)) > 0.95))
	lines.append("right_reverse_tip_axis_matches_reverse_contact_axis=%s" % str(float(right_reverse.get("tip_axis_score", 2.0)) > 0.95))
	lines.append("left_normal_tip_axis_matches_contact_axis=%s" % str(float(left_normal.get("tip_axis_score", -2.0)) > 0.95))
	lines.append("left_reverse_tip_axis_matches_reverse_contact_axis=%s" % str(float(left_reverse.get("tip_axis_score", 2.0)) > 0.95))
