...
const RESULT_FILE_PATH := "C:/WORKSPACE/stage2_feature_bridge_results.txt"
...
func _run_verification() -> void:
	...
	crafting_ui.open_for(player, forge_controller, "Stage2 Feature Bridge Bench")
	await process_frame
	await process_frame
	...
	var has_feature_bridge_fillet_tool: bool = _popup_has_item_text(geometry_popup, "Feature Bridge Fillet Tool")
	var has_feature_bridge_chamfer_tool: bool = _popup_has_item_text(geometry_popup, "Feature Bridge Chamfer Tool")
	var feature_bridge_restore_hidden: bool = not _popup_has_item_text(geometry_popup, "Feature Bridge Restore Tool")

	crafting_ui.call("_on_action_menu_id_pressed", int(menu_ids.get("geometry_tool_surface_feature_bridge_fillet", -1)))
	await process_frame
	await process_frame
	geometry_popup = crafting_ui.geometry_menu_button.get_popup()
	var feature_bridge_tool_apply_visible: bool = _popup_has_item_text(geometry_popup, "Apply Selected Targets")
	var feature_bridge_tool_clear_visible: bool = _popup_has_item_text(geometry_popup, "Clear Target Selection")

	var general_bridge_state = _build_synthetic_bridge_state(Stage2PatchStateScript.ZONE_GENERAL)
	var general_cluster_selection: Dictionary = selection_presenter.resolve_hover_selection_data(
		general_bridge_state,
	...
	var general_cluster_patch_ids: PackedStringArray = PackedStringArray(general_cluster_selection.get("patch_ids", PackedStringArray()))
	var general_bridge_ids: PackedStringArray = PackedStringArray(general_bridge_selection.get("bridge_ids", PackedStringArray()))
	...
	var bridge_chamfer_apply_patch_ids: PackedStringArray = selection_presenter.resolve_selection_apply_patch_ids(
		general_bridge_state,
		general_bridge_patch_ids,
		ForgeStage2SelectionPresenterScript.TOOL_STAGE2_SURFACE_FEATURE_BRIDGE_CHAMFER
	)
	var origins_before_bridge_chamfer: Array[Vector3] = _collect_patch_origins(general_bridge_state)
	var feature_bridge_chamfer_changed_shell: bool = crafting_ui.stage2_brush_presenter.apply_selection_tool(
		general_bridge_state,
		general_bridge_patch_ids,
		ForgeStage2SelectionPresenterScript.TOOL_STAGE2_SURFACE_FEATURE_BRIDGE_CHAMFER,
		bridge_chamfer_apply_patch_ids
	) and _has_any_patch_origin_changed(origins_before_bridge_chamfer, _collect_patch_origins(general_bridge_state))

	var bridge_restore_apply_patch_ids: PackedStringArray = selection_presenter.resolve_selection_apply_patch_ids(
		general_bridge_state,
		general_bridge_patch_ids,
		ForgeStage2SelectionPresenterScript.TOOL_STAGE2_SURFACE_FEATURE_BRIDGE_RESTORE
	)
	var origins_before_bridge_restore: Array[Vector3] = _collect_patch_origins(general_bridge_state)
	var feature_bridge_restore_changed_shell: bool = crafting_ui.stage2_brush_presenter.apply_selection_tool(
		general_bridge_state,
		general_bridge_patch_ids,
		ForgeStage2SelectionPresenterScript.TOOL_STAGE2_SURFACE_FEATURE_BRIDGE_RESTORE,
		bridge_restore_apply_patch_ids
	) and _has_any_patch_origin_changed(origins_before_bridge_restore, _collect_patch_origins(general_bridge_state))

	var grip_safe_bridge_state = _build_synthetic_bridge_state(Stage2PatchStateScript.ZONE_PRIMARY_GRIP_SAFE)
	var grip_safe_bridge_selection: Dictionary = selection_presenter.resolve_hover_selection_data(
		grip_safe_bridge_state,
		_build_synthetic_patch_hit_data(&"h_patch_0"),
		ForgeStage2SelectionPresenterScript.TOOL_STAGE2_SURFACE_FEATURE_BRIDGE_FILLET
	)
	var grip_safe_bridge_patch_ids: PackedStringArray = PackedStringArray(grip_safe_bridge_selection.get("patch_ids", PackedStringArray()))
	var grip_safe_bridge_fillet_apply_patch_ids: PackedStringArray = selection_presenter.resolve_selection_apply_patch_ids(
		grip_safe_bridge_state,
		grip_safe_bridge_patch_ids,
		ForgeStage2SelectionPresenterScript.TOOL_STAGE2_SURFACE_FEATURE_BRIDGE_FILLET
	)
	var origins_before_grip_safe_bridge_fillet: Array[Vector3] = _collect_patch_origins(grip_safe_bridge_state)
	var grip_safe_feature_bridge_fillet_changed_shell: bool = crafting_ui.stage2_brush_presenter.apply_selection_tool(
		grip_safe_bridge_state,
		grip_safe_bridge_patch_ids,
		ForgeStage2SelectionPresenterScript.TOOL_STAGE2_SURFACE_FEATURE_BRIDGE_FILLET,
		grip_safe_bridge_fillet_apply_patch_ids
	) and _has_any_patch_origin_changed(origins_before_grip_safe_bridge_fillet, _collect_patch_origins(grip_safe_bridge_state))

	var grip_safe_bridge_chamfer_apply_patch_ids: PackedStringArray = selection_presenter.resolve_selection_apply_patch_ids(
		grip_safe_bridge_state,
		grip_safe_bridge_patch_ids,
		ForgeStage2SelectionPresenterScript.TOOL_STAGE2_SURFACE_FEATURE_BRIDGE_CHAMFER
	)
	var origins_before_grip_safe_bridge_chamfer: Array[Vector3] = _collect_patch_origins(grip_safe_bridge_state)
	var grip_safe_feature_bridge_chamfer_changed_shell: bool = crafting_ui.stage2_brush_presenter.apply_selection_tool(
		grip_safe_bridge_state,
		grip_safe_bridge_patch_ids,
		ForgeStage2SelectionPresenterScript.TOOL_STAGE2_SURFACE_FEATURE_BRIDGE_CHAMFER,
		grip_safe_bridge_chamfer_apply_patch_ids
	) and _has_any_patch_origin_changed(origins_before_grip_safe_bridge_chamfer, _collect_patch_origins(grip_safe_bridge_state))

	var lines: PackedStringArray = []
	lines.append("stage2_geometry_menu_has_feature_bridge_fillet=%s" % str(has_feature_bridge_fillet_tool))
	lines.append("stage2_geometry_menu_has_feature_bridge_chamfer=%s" % str(has_feature_bridge_chamfer_tool))
	lines.append("stage2_geometry_menu_feature_bridge_restore_hidden=%s" % str(feature_bridge_restore_hidden))
	lines.append("feature_bridge_tool_apply_visible=%s" % str(feature_bridge_tool_apply_visible))
	lines.append("feature_bridge_tool_clear_visible=%s" % str(feature_bridge_tool_clear_visible))
	lines.append("synthetic_cluster_selected_count=%d" % general_cluster_patch_ids.size())
	lines.append("selected_bridge_count=%d" % general_bridge_ids.size())
	lines.append("synthetic_bridge_selected_count=%d" % general_bridge_patch_ids.size())
	lines.append("synthetic_bridge_larger_than_cluster=%s" % str(general_bridge_patch_ids.size() > general_cluster_patch_ids.size()))
	lines.append("feature_bridge_chamfer_changed_shell=%s" % str(feature_bridge_chamfer_changed_shell))
	lines.append("feature_bridge_restore_changed_shell=%s" % str(feature_bridge_restore_changed_shell))
	lines.append("grip_safe_feature_bridge_fillet_changed_shell=%s" % str(grip_safe_feature_bridge_fillet_changed_shell))
	lines.append("grip_safe_feature_bridge_chamfer_changed_shell=%s" % str(grip_safe_feature_bridge_chamfer_changed_shell))
...
func _build_wide_plate_wip() -> CraftedItemWIP:
	...
	wip.wip_id = &"verify_stage2_feature_bridge_wide_plate"
	wip.forge_project_name = "Verify Stage2 Feature Bridge Wide Plate"
