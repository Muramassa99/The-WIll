...
func resolve_preview_hand_mounted_motion_seed(
	...
	var requested_slide: float = float(resolved_seed.get(
	...
	_apply_preview_resolved_grip_state(held_item)
	if not unarmed_proxy:
		equipped_item_presenter.refresh_hand_mount_for_active_grip_seat(
			held_item,
			actor,
			_resolve_preview_dominant_slot_id(),
			requested_grip_style,
...
func _refresh_trajectory_visuals(
	...
	var authoring_drag_budgeted_visuals: bool = authoring_drag_active and bool(playback_state.get("authoring_drag_budgeted_visuals", false))
	...
	if not authoring_drag_lightweight and not authoring_drag_budgeted_visuals:
	...
	if authoring_drag_lightweight or authoring_drag_budgeted_visuals:
...
func _apply_authored_weapon_pose(
	...
	preview_root.set_meta("resolved_playback_state", resolved_playback_state)
	if (
		authoring_drag_active
		and not authoring_drag_lightweight
	):
		if actor != null:
			_apply_two_hand_preview_state(actor, held_item, selected_motion_node)
			_apply_preview_upper_body_authoring_state(actor, held_item, selected_motion_node, resolved_playback_state)
			_apply_preview_actor_upper_body_pose_now(actor, true)
		var live_drag_collision_result: Dictionary = _evaluate_preview_collision_pose(actor, held_item, held_item.global_transform)
		var live_drag_grip_error: float = _resolve_preview_grip_alignment_error(actor, held_item, _resolve_preview_dominant_slot_id())
		var live_drag_reason: String = (
			"live_drag_endpoint_authority"
			if use_free_authoring_endpoint_authority
			else "live_drag_constrained_endpoint"
		)
		var live_drag_metrics: Dictionary = {
			"stopped_reason": live_drag_reason,
			"weapon_locked_to_moving_hand": true,
			"pre_anchor_grip_error_meters": live_drag_grip_error,
			"grip_error_after_post_separation_meters": live_drag_grip_error,
			"post_anchor_separation_delta_meters": 0.0,
			"final_reseat_deferred_until_release": true,
		}
		preview_root.set_meta("contact_coupling_metrics", live_drag_metrics)
		preview_root.set_meta("contact_clearance_settle_metrics", live_drag_metrics)
		preview_root.set_meta("final_anchor_reseat_metrics", live_drag_metrics)
		preview_root.set_meta("collision_pose_legal", bool(live_drag_collision_result.get("legal", true)))
		preview_root.set_meta("collision_pose_deferred", false)
		preview_root.set_meta("collision_pose_illegal_sample_count", int(live_drag_collision_result.get("illegal_sample_count", 0)))
		preview_root.set_meta("collision_pose_region", String(live_drag_collision_result.get("colliding_body_region", "")))
		preview_root.set_meta("collision_pose_attachment", String(live_drag_collision_result.get("colliding_body_attachment_name", "")))
		preview_root.set_meta("collision_pose_sample", String(live_drag_collision_result.get("colliding_sample_name", "")))
		preview_root.set_meta("collision_pose_clearance_meters", float(live_drag_collision_result.get("estimated_clearance_meters", -1.0)))
		if update_camera:
			_update_camera(preview_root, weapon_grip_anchor_provider.get_primary_grip_anchor(held_item))
		return resolved_playback_state
	if authoring_drag_lightweight:
...
func _refresh_collision_debug_visuals(state: Dictionary) -> void:
	...
	var actor: Node3D = actor_pivot.get_node_or_null(PREVIEW_ACTOR_NAME) as Node3D if actor_pivot != null else null
	var held_item: Node3D = _get_node_meta_or_default(preview_root, "preview_held_item", null) as Node3D
	var collision_debug_visible: bool = (
		bool(_get_node_meta_or_default(preview_root, DEBUGGER_VIEW_ENABLED_META, false))
		and held_item != null
...
func _build_effective_preview_motion_node(
	...
	if playback_state.has("right_upperarm_roll_degrees"):
		effective_motion_node.right_upperarm_roll_degrees = float(playback_state.get(
			"right_upperarm_roll_degrees",
			effective_motion_node.right_upperarm_roll_degrees
		))
	if playback_state.has("left_upperarm_roll_degrees"):
		effective_motion_node.left_upperarm_roll_degrees = float(playback_state.get(
			"left_upperarm_roll_degrees",
			effective_motion_node.left_upperarm_roll_degrees
		))
...
func _apply_preview_motion_grip_state(
	...
	var requested_slide: float = float(playback_state.get(
		"grip_seat_slide_offset",
		motion_node.grip_seat_slide_offset if motion_node != null else CombatAnimationMotionNodeScript.DEFAULT_GRIP_SEAT_SLIDE_OFFSET
	))
...
func _apply_preview_actor_upper_body_pose_now(actor: Node3D, authoring_drag_active: bool = false) -> void:
	if actor == null:
		return
	if authoring_drag_active and actor.has_method("apply_authoring_preview_drag_frame_now"):
...
func _build_upperarm_roll_gizmo_state(
	...
	return {
		...
		"center_local": trajectory_root.global_transform.affine_inverse() * center_world,
		"handle_local": trajectory_root.global_transform.affine_inverse() * handle_world,
	}
...
func _draw_upperarm_roll_gizmo(
	...
	var center_local: Vector3 = roll_state.get("center_local", Vector3.ZERO) as Vector3
	var handle_local: Vector3 = roll_state.get("handle_local", Vector3.ZERO) as Vector3
