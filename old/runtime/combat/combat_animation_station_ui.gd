...
func set_selected_motion_node_upperarm_roll(
	...
	if _is_motion_node_authoring_locked(motion_node):
		_reject_locked_motion_node_edit()
		return false
	var resolved_roll: float = clampf(roll_degrees, -180.0, 180.0)
...
func _build_right_inspector(parent: HBoxContainer) -> void:
	...
	debugger_view_button = _build_styled_button(summary_content, "Debug View: ON")
...
func _refresh_editor_fields() -> void:
	...
	if motion_node_available:
		...
		grip_seat_slide_spin_box.value = CombatAnimationMotionNodeScript.DEFAULT_GRIP_SEAT_SLIDE_OFFSET
...
func _refresh_summary(status_message: String = "") -> void:
	...
	if active_wip == null:
		...
		lines.append("Select a saved weapon or - Unarmed to begin authoring.")
	else:
		var station_state: Resource = _get_active_station_state()
		var draft: Resource = _get_active_draft()
		var motion_node: CombatAnimationMotionNode = _get_active_motion_node()
...
func _queue_preview_drag_refresh() -> void:
	...
	if preview_drag_first_pending_msec <= 0:
...
func _flush_pending_preview_drag_refresh() -> void:
	...
	if not motion_node_editor.is_dragging():
		...
		return
	var now_msec: int = Time.get_ticks_msec()
	if preview_drag_last_refresh_msec <= 0 and preview_drag_first_pending_msec > 0:
		if now_msec - preview_drag_first_pending_msec < PREVIEW_DRAG_INITIAL_SOLVE_DELAY_MSEC:
			return
...
func _resolve_active_unarmed_geometry_seed_base() -> Dictionary:
	...
	unarmed_geometry_seed["grip_seat_slide_offset"] = CombatAnimationMotionNodeScript.DEFAULT_GRIP_SEAT_SLIDE_OFFSET
...
func _draft_matches_raw_weapon_geometry_baseline(draft: Resource) -> bool:
	...
	var seeded_grip_slide: float = float(authored_seed.get(
...
func _apply_motion_node_state(target: CombatAnimationMotionNode, source: CombatAnimationMotionNode) -> bool:
	...
	if not target.pommel_curve_out_handle.is_equal_approx(source.pommel_curve_out_handle):
		...
		changed = true
	if not is_equal_approx(target.right_upperarm_roll_degrees, source.right_upperarm_roll_degrees):
		target.right_upperarm_roll_degrees = source.right_upperarm_roll_degrees
		changed = true
	if not is_equal_approx(target.left_upperarm_roll_degrees, source.left_upperarm_roll_degrees):
		target.left_upperarm_roll_degrees = source.left_upperarm_roll_degrees
		changed = true
	target.normalize()
	return changed
...
func _refresh_focus_indicators() -> void:
	...
	var arm_roll_active: bool = session_state.is_arm_roll_focused()
...
func _handle_preview_drag(screen_position: Vector2) -> void:
	...
	if drag_target == CombatAnimationMotionNodeEditorScript.DRAG_TARGET_TIP:
		...
		if resolved_upperarm_roll != null:
			...
			var next_roll: float = clampf(float(resolved_upperarm_roll), -180.0, 180.0)
