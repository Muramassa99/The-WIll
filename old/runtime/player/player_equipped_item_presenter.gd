...
const ACTIVE_PRIMARY_GRIP_SEAT_LOCAL_META := "active_primary_grip_seat_local"
...
func apply_held_item_grip_style_mode(
	...
	refresh_hand_mount_for_active_grip_seat(
		held_item,
		humanoid_rig,
		slot_id,
		resolved_grip_style,
		_resolve_active_primary_grip_seat_local(held_item),
		true
	)

func refresh_hand_mount_for_active_grip_seat(
	held_item: Node3D,
	humanoid_rig: Node3D,
	slot_id: StringName,
	grip_style_mode: StringName = StringName(),
	primary_grip_seat_local: Vector3 = Vector3.INF,
	apply_when_hand_mounted: bool = true
) -> void:
	if held_item == null or not is_instance_valid(held_item):
		return
	var grip_style_seed: StringName = grip_style_mode
	if grip_style_seed == StringName():
		grip_style_seed = held_item.get_meta("grip_style_mode", CraftedItemWIP.GRIP_NORMAL) as StringName
	var resolved_grip_style: StringName = CraftedItemWIP.normalize_grip_style_mode(grip_style_seed)
	held_item.set_meta("grip_style_mode", resolved_grip_style)
	var local_tip: Vector3 = held_item.get_meta("weapon_tip_local", Vector3.ZERO) as Vector3
	var resolved_grip_seat_local: Vector3 = (
		_resolve_active_primary_grip_seat_local(held_item)
		if _is_vector3_inf(primary_grip_seat_local)
		else primary_grip_seat_local
	)
	held_item.set_meta(ACTIVE_PRIMARY_GRIP_SEAT_LOCAL_META, resolved_grip_seat_local)
	...
	var resolved_basis: Basis = _resolve_signed_contact_axis_weapon_hold_basis(
		...
		resolved_grip_style,
		local_tip,
		resolved_grip_seat_local,
		fallback_basis
	)
	var mount_transform: Transform3D = resolve_hand_mount_local_transform(held_item)
	var target_contact_local: Vector3 = mount_transform.origin + mount_transform.basis * resolved_grip_seat_local
	if humanoid_rig != null and humanoid_rig.has_method("resolve_hand_grip_alignment_offset_local"):
		var alignment_variant: Variant = humanoid_rig.call("resolve_hand_grip_alignment_offset_local", slot_id)
		if alignment_variant is Vector3:
	...
	if not apply_when_hand_mounted:
		return
	var hand_anchor: Node3D = get_hand_anchor(humanoid_rig, slot_id)
	if hand_anchor != null and held_item.get_parent() == hand_anchor:
		held_item.transform = mount_transform

func _resolve_active_primary_grip_seat_local(held_item: Node3D) -> Vector3:
	if held_item == null:
		return Vector3.ZERO
	var fallback: Vector3 = held_item.get_meta("primary_grip_contact_local", Vector3.ZERO) as Vector3
	if held_item.has_meta(ACTIVE_PRIMARY_GRIP_SEAT_LOCAL_META):
		var active_variant: Variant = held_item.get_meta(ACTIVE_PRIMARY_GRIP_SEAT_LOCAL_META, fallback)
		if active_variant is Vector3:
			return active_variant
	if held_item.has_meta(PREVIEW_PRIMARY_GRIP_SEAT_LOCAL_META):
		var preview_variant: Variant = held_item.get_meta(PREVIEW_PRIMARY_GRIP_SEAT_LOCAL_META, fallback)
		if preview_variant is Vector3:
			return preview_variant
	return fallback

func _is_vector3_inf(value: Vector3) -> bool:
	return is_inf(value.x) or is_inf(value.y) or is_inf(value.z)
