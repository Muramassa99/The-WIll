...
func _apply_lightweight_runtime_clip_pose(
	...
	if authored_tip_world.distance_to(authored_pommel_world) > 0.000001:
		...
		solved_transform = weapon_frame_solver.solve_transform_from_segment(
			local_tip,
			local_pommel,
			authored_tip_world,
	...
	live_pose_presenter._apply_two_hand_preview_state(
		humanoid_rig,
		held_item,
		effective_motion_node
	)
	live_pose_presenter._apply_preview_upper_body_authoring_state(
		humanoid_rig,
		held_item,
		effective_motion_node,
	...
	live_pose_presenter._apply_preview_actor_upper_body_pose_now(humanoid_rig)
